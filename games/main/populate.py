...
def extraerDatosURL():
    ...
    pc = urlopen('https://www.clavecd.es/catalog/category-pc-games-all/').read()
    ...
    for game in gamesPc:
        ...
        pc = urlopen(link2,timeout=time).read()
        soup = BeautifulSoup(pc, 'lxml')
        ...
        Game.objects.create(title=name,price=float(price.split('€')[0]),category=category,comments=int(comments),rate=float(rate),plataform='PC',link=link2,picture=picture,gamePrice=float(priceG))
    ...
    for game in gamesXbox:
        ...
        if(len(priceG)==0):
            ...
            priceG = priceG[0].get_text()
            priceG = priceG.replace(",", ".")
        Game.objects.create(title=name,price=float(price.split('€')[0]),category=category,comments=int(comments),rate=float(rate),plataform=plataform,link=link2,picture=picture,gamePrice=float(priceG))
    
    #PSX Games -------------------------------------------------------------------------------------------------------------------
    
    psx = urlopen('https://www.keyforsteam.de/catalogue/category-playstation-all/').read()
    ...
    nintendo = urlopen('https://cheapdigitaldownload.com/catalog/category-nintendo-all/').read()
    ...
    for game in gamesNintendo:
        ...
        if(len(priceG)==0):
            ...
            priceG = priceG[0].get_text()
            priceG = priceG.replace(",", ".")
        Game.objects.create(title=name,price=float(price.split('$')[1]),category=category,comments=int(comments),rate=float(rate),plataform=plataform,link=link2,picture=picture,gamePrice=float(priceG))
